...
for name in ['numpy','scipy','pillow','espeakng-loader','imageio-ffmpeg','num2words','pyinstaller','docopt','rapidocr-onnxruntime','onnxruntime','opencv-python','shapely','pyclipper','PyYAML','protobuf','faster-whisper','ctranslate2','av','tokenizers','huggingface-hub','requests']:
...
subprocess.run([str(dest/'AutoEditor.exe'),'--self-test',str(report)],check=True,timeout=120)
result=json.loads(report.read_text(encoding='utf-8'))
if result.get('status')!='ok':raise RuntimeError(result)
subprocess.run([str(dest/'AutoEditor.exe'),'--smoke-gui'],check=True,timeout=45)
shutil.copy2(report,dest/'bundle-check.json')
shutil.make_archive(str(root/'dist'/f'AutoEditor-{__version__}-Windows'),'zip',root/'dist','AutoEditor')
